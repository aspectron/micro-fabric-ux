...
let build = ()=>{
  ...
  var basePath = "/@BUILD/node_modules/";
  ...
  let buildStream = mergeStream(sourcesStream, dependenciesStream)
    .on('data', (e, d) => {
      ...
      c = c.replace(/'@polymer/g, "'"+basePath+"@polymer").replace(/"@polymer/g, '"'+basePath+'@polymer')
           .replace(/'@webcomponents/g, "'"+basePath+"@webcomponents").replace(/"@webcomponents/g, '"'+basePath+'@webcomponents')
      e.contents = new Buffer(c);
      console.log('Analyzing build dependencies...', e.path);
    });

  // If you want bundling, pass the stream to polymerProject.bundler.
  // This will bundle dependencies into your fragments so you can lazy
  // load them.
  //buildStream.pipe(gulpif(/\.js$/, uglify()))
